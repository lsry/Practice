...
证明：
* 循环不变式：A[1 , ... , i - 1]中不存在等于 v 的元素
* 初始化：当 i=1 时，A[0] 为空，不存在等于 v 的元素
* 保持：当 i 移动时 ，前面 i - 1 个数已经被查找过了，如果等于 v 就会返回i 否则继续查找，因此前面 A[1 , ... , i - 1] 个数在循环中不存在等于 v 的元素  
* 终止：导致 for 终止条件为存在 A[i] = v ，此时算法正确 否则扫描完成，A 中的所有元素均被查找过，不存在 A[i] = v，则返回NIL 因此算法过程正确。
...
运行时间：Σ(j-1) = n(n-1)/2 = Θ(n^2)

- c 当 i=0 时终止，在这循环开始前，y=Σ[a(k+1) * x^k] (0<=k<=n-1)   
循环里：y = a0 + x*Σ[a(k+1)x^k] = Σ[ak*x^k] (0<=k<=n)

- d  
初始化：当i = n, y=Σ[a(k+n+1)x^k] (0<=k<=-1) => y=0  
保持：在这循环开始前，y=Σ[a(k+1) * x^k] (0<=k<=n-i-1)   
i=i+1 y=a(i+1) + xΣ[a(k+1) * x^k]，因此前面已经计算好的部分是不变的。
终止：如c。

#### 2-4
- a (2,1) (3,1) (8,6) (8,1) (6,1)

- b 以逆序排列的数组 有 (n-1) * n/2 个

- c 相同 假设在最坏情况，在插入排序内层循环 ，只有与关键值逆序的数才会被移动位置，此时 时间为(i-1) (1<i<n-1),外层总共执行n-1次，可得时间为 n*(n-1)/2
